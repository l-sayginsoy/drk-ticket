...
import { Ticket, Status, Priority, Role, User, AppSettings, AvailabilityStatus, StaffMessage, WorkTimeEntry } from '../types';
...
const TicketDetailSidebar: React.FC<TicketDetailSidebarProps> = ({ ticket, onClose, onUpdateTicket, users, statuses, currentUser, appSettings }) => {
    ...
    const workTimeEntries = ticket.workTimeEntries || [];
    const totalWorkMinutes = workTimeEntries.reduce((sum, entry) => sum + entry.minutes, 0);
    ...
    const updateWorkTimeEntries = (entries: WorkTimeEntry[]) => {
        const totalMinutes = entries.reduce((sum, entry) => sum + entry.minutes, 0);
        onUpdateTicket({
            ...ticket,
            workTimeEntries: entries,
            costs: {
                laborHours: Math.round((totalMinutes / 60) * 100) / 100,
                materials: ticket.costs?.materials || 0,
            },
        });
    };
                ...
                {isEditing ? (
                    <div style={{ position: 'relative' }}>
                      ...
                      <div style={{ position: 'absolute', bottom: 6, right: 6 }}>
                        <MicButton value={editDraft.description || ''} onChange={v => setEditDraft(d => ({ ...d, description: v }))} title="Per Sprache eingeben" />
                      </div>
                    </div>
                ) : (
                ...
                {chatOpen && (
                  <div className="channel-body">
                    ...
                    <div className="channel-input-row">
                      ...
                      />
                      <MicButton value={newStaffMsg} onChange={setNewStaffMsg} title="Per Sprache eingeben" />
